...
public class DamageIndicator {
    ...
    public static void render(PoseStack pose, float tickDelta) {
        ...
        synchronized (indicators) {
            ...
            if (useCaptured) {
                ...
                viewMatrix.translate(new Vector3f((float) -camPos.x, (float) -camPos.y, (float) -camPos.z));
                Matrix4f projectionMatrix = capturedProjection != null
                    ? new Matrix4f(capturedProjection)
                    : Matrix4f.perspective(Math.toRadians(client.options.fov), aspect, 0.05f, 1000.0f);
            ...
            for (RenderedIndicator ri : toRender) {
                ...
                if (config.indicatorBold) {
                    ...
                    var boldText = new TextComponent(ri.text).withStyle(ChatFormatting.BOLD).getVisualOrderText();
